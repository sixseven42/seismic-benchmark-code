"""DNNDAT multiples attenuation inference: noisy SEG-Y → denoised SEG-Y.

Example::

    python scripts/multiples_attenuation/inference/inference_denoise_dnndat.py \\
        --config configs/multiples_attenuation/denoise_dnndat.yaml \\
...
_DEFAULT_CONFIG = "configs/multiples_attenuation/denoise_dnndat.yaml"
...
def main() -> None:
    p = argparse.ArgumentParser(description="DNNDAT multiples attenuation inference.")
    from _common import add_inference_args, run_inference
    add_inference_args(p)
    args = p.parse_args()
    run_inference(args, _DEFAULT_CONFIG, build_model, _forward, model_type_label="dnndat")
