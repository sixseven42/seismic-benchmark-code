"""UNet multiples attenuation inference: noisy SEG-Y → denoised SEG-Y.

Example::

    python scripts/multiples_attenuation/inference/inference_denoise_unet.py \\
...
_DEFAULT_CONFIG = "configs/multiples_attenuation/denoise_unet.yaml"


def _forward(model: torch.nn.Module, batch: torch.Tensor) -> torch.Tensor:
    """UNet predicts multiples noise; denoised = input - predicted noise."""
    return batch - model(batch)


def main() -> None:
    p = argparse.ArgumentParser(description="UNet multiples attenuation inference.")
