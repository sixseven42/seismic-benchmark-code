...
MASTER_PORT=29700          # base port for torchrun; incremented per run to avoid EADDRINUSE
TORCHRUN_EXTRA=""          # optional: extra flags for torchrun, e.g. "--standalone"
# ------------------------------------

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
BASE_CONFIG="${REPO_ROOT}/configs/ground_roll_attenuation/field/denoise_pix2pix.yaml"
PY_SCRIPT="${REPO_ROOT}/scripts/ground_roll_attenuation/train/train_denoise_pix2pix.py"
