"""Physics-DNN ground-roll attenuation inference: noisy SEG-Y → denoised SEG-Y.

Example::

    python scripts/ground_roll_attenuation/inference/inference_denoise_physics_dnn.py \\
...
_DEFAULT_CONFIG = "configs/ground_roll_attenuation/denoise_physics_dnn.yaml"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Physics-DNN ground-roll attenuation inference.")
    from _common import add_inference_args, run_inference
    add_inference_args(p)
    args = p.parse_args()
    run_inference(args, _DEFAULT_CONFIG, build_model, _forward, model_type_label="physics_dnn")
