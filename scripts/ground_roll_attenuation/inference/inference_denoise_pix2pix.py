"""Pix2Pix ground-roll attenuation inference: noisy SEG-Y → denoised SEG-Y.

The Generator predicts the ground-roll noise component.
Denoised signal = noisy_input - predicted_noise.

Example::

    python scripts/ground_roll_attenuation/inference/inference_denoise_pix2pix.py \\
...
_DEFAULT_CONFIG = "configs/ground_roll_attenuation/denoise_pix2pix.yaml"


def _forward(model: torch.nn.Module, batch: torch.Tensor) -> torch.Tensor:
    """Generator predicts noise component; denoised = input - predicted noise."""
    return batch - model(batch)


def main() -> None:
    p = argparse.ArgumentParser(description="Pix2Pix ground-roll attenuation inference.")
    from _common import add_inference_args, run_inference
    add_inference_args(p)
    args = p.parse_args()
    run_inference(args, _DEFAULT_CONFIG, build_model, _forward, model_type_label="pix2pix")
