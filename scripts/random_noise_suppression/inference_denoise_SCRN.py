"""Random-noise suppression inference: SCRN full-shot denoising, per-shot metrics, and viz."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/random_noise_suppression/denoise_SCRN.yaml",
...
def main() -> None:
    ...
    n_viz_shots = args.n_viz_shots if args.n_viz_shots is not None else infer_cfg.get("n_viz_shots", 5)
    ...
    save_npy = args.save_npy if args.save_npy is not None else infer_cfg.get("save_npy", False)

    prep = cfg.get("preprocess", {})
    noise_kind = args.noise_kind if args.noise_kind is not None else str(prep.get("noise_kind", "gaussian"))
    snr_db = args.snr_db if args.snr_db is not None else float(prep.get("snr_db", 20.0))
    ...
    with csv_path.open("w", encoding="utf-8") as f:
        ...
        for i in range(n_shots):
            row = [str(i)]
            row += [format_metric_value(k, float(noisy_per_shot[k][i])) for k in noisy_per_shot.keys()]
            row += [format_metric_value(k, float(denoised_per_shot[k][i])) for k in denoised_per_shot.keys()]
            row += [format_metric_value(k, float(delta_per_shot[k][i])) for k in delta_per_shot.keys()]
            f.write(",".join(row) + "\n")
