...
Example (single GPU):
    python scripts/random_noise_suppression/inference_denoise_unet.py \
        ...
        --checkpoint results/random_noise_unet_base/checkpoints/epoch_0049.pt \
        --output-dir results/random_noise_unet_base/inference \
        --noise-kind gaussian \
        --snr-db 5 \
        --n-viz-shots 5
"""
...
def _resolve_data_cfg(cfg: Dict[str, Any], infer_cfg: Dict[str, Any]) -> Dict[str, Any]:
    """Pick inference.data first, then fall back to training data."""
...
def _extract_per_shot_ffid(data_cfg: Dict[str, Any], n_shots: int) -> np.ndarray:
    """Return one FFID per shot; fall back to shot index for non-SEG-Y inputs."""
...
def _select_inference_shots(
    ...
    """Optionally select only the inference test split from the raw volume."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/random_noise_suppression/denoise_unet.yaml",
